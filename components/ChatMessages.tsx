...
const icons = {
  ai: (
    <div className="w-8 h-8 sm:w-9 sm:h-9 bg-blue-500 rounded-full flex items-center justify-center shadow-md mt-2">
  ...
  user: (
    <div className="w-8 h-8 sm:w-9 sm:h-9 bg-gray-600 rounded-full flex items-center justify-center shadow-md">
          ...
          return (
            <div key={id} className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-[90%] sm:max-w-[70%]`}>
                <div className={`rounded-2xl px-3 py-1.5 sm:px-4 sm:py-2 shadow ${isUser ? 'bg-blue-500 text-white rounded-br-md' : 'bg-gray-700 text-white rounded-bl-md'}`}>
                  <p className="whitespace-pre-wrap break-words text-sm sm:text-base leading-relaxed">{content}</p>
                </div>
                <span className={`text-[11px] sm:text-xs text-gray-400 mt-1 mb-1 ${isUser ? 'pr-1' : 'pl-1'}`}>
      ...
      {isLoading && (
        <div className="flex w-full justify-start mt-2">
          ...
          <div className="max-w-[90%] sm:max-w-[70%] flex items-center">
            <div className="bg-gray-700 rounded-2xl px-3 py-1.5 sm:px-4 sm:py-2 shadow">
